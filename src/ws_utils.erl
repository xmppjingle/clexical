...
get_cdata(#xmlel{children = [{xmlcdata, CData}]}) ->
    CData;
...
headers_to_attribs(Headers) -> headers_to_attribs(Headers, []).

headers_to_attribs([], Buf) -> Buf;
headers_to_attribs([#xmlel{attrs=[{?ATTRIB_NAME, N},{?ATTRIB_VALUE, V}]}|T], Buf) -> 
  headers_to_attribs(T, [{N, V}|Buf]);
headers_to_attribs([_|T], Buf) ->
  headers_to_attribs(T, Buf).