...
%%
%% Uses OTP inets/httpc — no external HTTP dependency required.
%% The proxy is configured automatically from HTTPS_PROXY / HTTP_PROXY env vars.
%%
...
call(Opts, AttemptsLeft) ->
    ...
    case httpc:request(Method, Request, HttpOpts, ReqOpts) of
...
ensure_httpc() ->
    ...
    %% honour HTTP_PROXY / HTTPS_PROXY env vars
...
set_proxy(ProxyUrl) ->
    case uri_string:parse(list_to_binary(ProxyUrl)) of
        #{host := Host, port := Port} ->
            httpc:set_options([{proxy, {{binary_to_list(Host), Port}, []}}]);
        #{host := Host} ->
            httpc:set_options([{proxy, {{binary_to_list(Host), 8080}, []}}]);
        _ ->
            ok
    end.
...
%% GET requests don't have a body in httpc's API
...
render_body(Template, Ctx) when is_binary(Template) ->
    try
        bbmustache:render(Template, Ctx, [{key_type, binary}])
    catch
        _:_ -> Template
    end.
